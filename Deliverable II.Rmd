...
```{r}
GarageandBasementVsAssessmentValue = lm(formula = assessment_value_town~factor(garage) + factor(basement), data = train)
...
```{r}
GarageandBasementVsClosingPrice = lm(formula = price_closed~factor(garage) + factor(basement), data = train)
summary(GarageandBasementVsClosingPrice)
```
This code regresses garage and basement against closing price. The independent variable is the presence of basement and/or garage, and the dependent variable is closing price. The coefficient for garage is 315049, meaning that a house with a garage, holding basement constant, has a closing price $315049 higher than one without a garage. The p-value is 0.0275, meaning this coefficient is significant. Therefore, we reject the null hypothesis that the presence of a garage does not affect the closing price of a house, holding basement constant. The coefficient for basement is -37740, meaning that a house with a basement, holding garage constant, has a closing price $37740 less than a house without a basement. However, the p-value is 0.8534, meaning this coefficient is not significant and we faill to reject the null hypothesis that the presence of a basement does not affect the closing price of a house, holding garage constant. The r-squared is 0.03672, which is the amount of variation in closing price that can be accounted for by presence of a garage and/or basement. 
