...
While closing price and assesment value follow the same general trends, closing price is lower. This plot shows overlaying density plots for assessment value and closing price. We see that while closing price is generally higher than assesment value, the two are fairly similar. This suggests town assessment value is a reasonable predictor of closing price, but does tend to understimate prices. 
```{r out.width = "50%"}
plot(density(train$price_closed), 
     main = "Overlaying density plots Assesment Value and Closing Price", 
     xlab = "Price/Value ($)", col = "gray70")
lines(density(na.exclude(train$assessment_value_town)), col = "gray20")
legend("topright", c("Closing Price", "Assessment Value"), fill = c("gray70",
"gray20"))
```
...
plot(train$rooms_total, train$price_closed, 
     main = "Scatter Plot of Bedrooms vs. Price", 
...
```{r results = 'asis'}
vectorlm_1 = c("test_lm_45", 0.8777, 3608.411, 3640.038)
vectorlm_2 = c("test_lm_46", 0.878, 3609.041, 3643.543)
vectorlm_3 = c("test_lm_47", 0.8859, 3601.211, 3638.588)
...
summary(test_lm_47)
```
This model uses square feet interacted with whether the house is in Pomfret, tax amount interacted with whether the house is in Pomfret, whether the house is next to a river interacted with the number of acres of the property, garage capacity, tax interacted with the number of acres of the property, and tax interacted with garage capacity. As we can see from the coefficients of the regression, a unit increase in square feet produces a $108.60 decrease in closing price. The p-value of this coefficient is less than 0.01, meaning it is significant. A unit increase in tax produces a $66.12 increase in closing price, an increase in garage capacity produces a $52,580 decrease in closing price, a unit increase in tax for a house in Pomfret produces a $33.62 unit decrease in closing price, a house with a unit increase in acres produces a $84,850 increase in closing price for a house next to a river rather than not next to a river, and a house with an increase in garage capacity has a $4.22 increase in closing price when interacted with acres, meaning the effect of garage capacity is amplified in properties with more acres.
```{r}
library(readr)
...
prediction.error45 = sqrt(mean((na.exclude(predicted.price.45)-validate$price_closed)^2))
prediction.error45

predicted.price.46 = predict(test_lm_46, validate)
prediction.error46 = sqrt(mean((na.exclude(predicted.price.46)-validate$price_closed)^2))
prediction.error46

predicted.price.47 = predict(test_lm_47, validate)
prediction.error47 = sqrt(mean((na.exclude(predicted.price.47)-validate$price_closed)^2))
prediction.error47
```
...
summary(test_lm_45)
```
Test 45 includes the following explanatory variables: square feet interacted with whether or not the house is in Pomfret, tax interacted with whether or not the house is in Pomfret, whether the house in Pomfret, garage capacity, and whether the house is near a river interacted with acres. We see that a unit increase in square feet results in a $102.10 decrease in closing price. The p-value is 0.000198 which means it is significant. Similarly, a unit increase in tax results in a $77.12 increase in closing price, which has a p-value of <.01 and is therefore significant. Houses in Pomfret experience a $39.23 decrease in closing price per unit change in tax as compared to houses not in Pomfret -- this coefficient has a p-value of <.01 and is significant. Lastly, houses near a river see a $82,860 increase in closing price when there is a one unit change in acres, with a p-value of <.01 and therefore a significant coefficient. 
...
Pomfret$PredictionError = abs(Pomfret$price_closed 
       - Pomfret$predicted)/Pomfret$price_closed *100

Woodstock$PredictionError = abs(Woodstock$price_closed 
        - Woodstock$predicted)/Woodstock$price_closed *100

Barnard$PredictionError = abs(Barnard$price_closed 
        - Barnard$predicted)/Barnard$price_closed *100
...
```
This code regresses garage and basement against assessment deviation. The independent variable is the presence of basement and/or garage, and the dependent variable is the assessment deviation. The coefficient for garage is $47,516, meaning that a house with a garage, holding basement constant, has an assessment deviation value of $47,516 higher than one without a garage. However, the p-value is 0.620, meaning this coefficient is not significnat and we fail to reject the null hypothesis that the presence of a garage does not affect the assessment deviation of a house, holding basement constant. The coefficient for basement is -$155,902, meaning that a house with a basement, holding garage constant, has an assessment deviation value of $155,902 less than one without a basement. However, the p-value is 0.252, meaning this coefficient is not significant and we fail to reject the null hypothesis that the presence of a basement does not affect the assessment deviation of a hosue, holding garage constant. The r-squared is 0.01142, which is the amount of variation in assessment deviation that can be accounted for by the presence of a garage and/or basement. 
...
validate$garage_type[is.na(validate$garage_type)] = "Unknown"
validate$isGarageDetached = 0
validate[validate$garage_type == "Detached",]$isGarageDetached = "Yes"

validate$short_sale[is.na(validate$short_sale)] = "Yes"
...
PredictionError_Data = subset(validate, select = c("id", "assessment_deviation"))
PredictionError_Data$predicted.deviation.34 = predicted.deviation.34
PredictionError_Data$predicted.deviation.35 = predicted.deviation.35
PredictionError_Data$predicted.deviation.36 = predicted.deviation.36

Clean_PredictionError_Data = PredictionError_Data[complete.cases(PredictionError_Data),]

prediction.error34 = sqrt(mean((Clean_PredictionError_Data$predicted.deviation.34-Clean_PredictionError_Data$assessment_deviation)^2))
prediction.error34

prediction.error35 = sqrt(mean((Clean_PredictionError_Data$predicted.deviation.35-Clean_PredictionError_Data$assessment_deviation)^2))
prediction.error35

prediction.error36 = sqrt(mean((Clean_PredictionError_Data$predicted.deviation.36-Clean_PredictionError_Data$assessment_deviation)^2))
prediction.error36
```
We choose model 34 because it has the lowest prediction error.

```{r results = 'asis'}
vectorlm_4 = c("trial_lm34", 0.5319, 3451.648, 3496.643)
vectorlm_5 = c("trial_lm35", 0.5319, 3451.648, 3500.569)
vectorlm_6 = c("trial_lm36", 0.531, 3453.587, 3504.207)
lm_chart = rbind(vectorlm_4, vectorlm_5, vectorlm_6)
colnames(lm_chart) = c("Regression", "Adjusted R-squared", "AIC", "BIC")
knitr::kable(lm_chart, caption = "Alternate Regressions")
```
...
summary(trial_lm34)
```
This model uses tax, total stories, short sale, acres, square feet, rooms, square feet interacted with whether the house is in Pomfret, tax interacted with whether the house is in Pomfret, the number of acres interacted with whether there is a river, tax interacted with number of acres, whether the garage is detached interacted with garage capacity, square feet interacted with acres, and total stories interacted with rooms. The model allows the interacted terms to have different slopes, but uses the same intercept for interacted terms. The coefficient for tax is 20.12 and is significant (p<.05), meaning there is a $20.12 increase in assessment deviation with a unit increase in taxes. The coefficient for river interacted with acres is 87820, meaning there is a $87,820 increase in assessment deviation for each unit increase in acres for houses with rivers. This coefficient is significant (p<.05). 
...
validate$predicted2 = predict(trial_lm34, validate)

Pomfret = subset(validate, city == "Pomfret")
Woodstock = subset(validate, city == "Woodstock")
Barnard = subset (validate, city == "Barnard")

Pomfret$PredictionError = abs(Pomfret$assessment_deviation 
        - Pomfret$predicted2)/Pomfret$assessment_deviation *100

Woodstock$PredictionError = abs(Woodstock$assessment_deviation 
        - Woodstock$predicted2)/Woodstock$assessment_deviation *100

Barnard$PredictionError = abs(Barnard$assessment_deviation 
        - Barnard$predicted2)/Barnard$assessment_deviation *100

plot(density(na.exclude(Pomfret$PredictionError)), xlim=c(-10 , 125), ylim = c(0 , 0.003), col = "gray70" , main = "Assessment Deviation Percent Error by Town" , xlab ="Prediction Percent Error")
lines(density(na.exclude(Woodstock$PredictionError)) , col = "gray40")
lines(density(na.exclude(Barnard$PredictionError)) , col = "gray8")
legend("topright" , c("Pomfret","Woodstock", "Barnard"), fill=c("gray70", "gray40" , "gray8") )
```
The assessment deviation prediction error varies greatly. This is because assessment deviation is highly volatile and difficult to predict compared to the closing price of a house. For example, if a house is worth $100,000 and the town predicts the assessment value to be $100,500, then the deviation is $500. If the predicted assessment deviation is $100, but the predicted closing price is $100,100. The percent error of the closing price is very small, but the percent error for the assessment deviation is 80%. So, while assessment deviation prediction can be used to predict the value of a house in tandem with the town assessment value variable, it is not a great estimator on its own.

*Does the mean % error decrease by using assessment deviation instead?*

The follow code finds that our mean % error using the initial model was 23.19%. 
```{r}
...
```
```{r}
PercentADError = subset(validate, select = c("assessment_deviation", "predicted2"))
Clean_PercentADError = PercentADError[complete.cases(PercentADError),]
View(Clean_PercentADError)
Clean_PercentADError$percent_error = ((Clean_PercentADError$assessment_deviation - Clean_PercentADError$predicted2)/Clean_PercentADError$assessment_deviation)*100
Clean_PercentADError$abs_percent_error = abs(Clean_PercentADError$percent_error)
View(Clean_PercentADError)
mean(Clean_PercentADError$abs_percent_error)
```
The mean percent error increases by using the assessment deviation instead of the initial closing price model. Because the new model does not use one of the most powerful explanatory variables - town assessment value - from the initial model, the predictive power of the new model is greatly decreased.

```{r}
PercentCPError = subset(validate, select = c("price_closed", "assessment_deviation", "predicted2", "assessment_value_town"))
Clean_PercentCPError = PercentCPError[complete.cases(PercentCPError),]
Clean_PercentCPError$percent_error = (((Clean_PercentCPError$price_closed - (Clean_PercentCPError$assessment_value_town + Clean_PercentCPError$predicted2))/Clean_PercentCPError$price_closed)*100)
Clean_PercentCPError$abs_percent_error = abs(Clean_PercentCPError$percent_error)
View(Clean_PercentCPError)
mean(Clean_PercentCPError$abs_percent_error)
```
This is the mean percent error of closing price using the new model. We added our predicted assessment deviation to the town assessment value. The reason the percent error is lower is because it is less sensitive to small fluctuations. While it is substantially better than the percent error of the predicted assessment deviation, the original model predicting closing price is still a better predictor of closing price. 
