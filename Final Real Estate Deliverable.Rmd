...
plot(density(na.exclude(Pomfret$PredictionError)), xlim=c(-10 , 125), ylim = c(0 , 0.03), col = "gray70" , main = "Percent Error by Town" , xlab ="Prediction Percent Error")
...
trial_lm30=lm(data=train,assessment_deviation~sq_ft_tot_fn*factor(isPomfret)+tax_gross_amount*factor(isPomfret)+factor(isRiver)*acres+tax_gross_amount*acres+isGarageDetached*garage_capacity+total_stories+short_sale+acres*sq_ft_tot_fn)

predictedCP = predict(trial_lm30, test, se.fit = TRUE)
test$predictedCP = predict(trial_lm30, test)
test$se.fit = predictedCP$se.fit
test$upper.limit = test$predictedCP + qnorm(0.975) * test$se.fit
test$lower.limit = test$predictedCP - qnorm(0.975) * test$se.fit
test$expected.profit = (.08) * test$predictedCP
test$profit.upper.limit = (.08) * test$upper.limit
test$profit.lower.limit = (.08) * test$lower.limit
...
```
With 95% confidence, total revenue on the test data lies between -$188,734.4 and $80.758.41.
